...
if __name__ == '__main__':
    conn = pymysql.connect(host='10.100.94.71', db='dp', user='cm', passwd='cm-ops', port=int(3306))
    #now = datetime.datetime.now()
    ihour=(datetime.datetime.now()-datetime.timedelta(hours=0)).strftime("%H")
    fsql='''SELECT * from upstream_table_monitor_hi where err_hour="{}";'''.format(ihour)
    ...
    for i in range(len(df)):
        ...
        if  path:
            table_ds=df.iloc[i]['err_ds']
            idate=getYesterday(int(table_ds))
            dsdate='ds={}'.format(idate)
            lenght=get_table_length(namenode_enable,50070,path,dsdate)
            if lenght <= 100 :
                send_params['card']['elements'][0]['fields'][3]['text']['content']='** 延迟表**\n{} \n**表大小:** {}\n**项目：** {}\n**工作流** {}\n**任务节点** {}\n**表类型** {}'.format(df.iloc[i]['table_name'],lenght,df.iloc[i]['project'],df.iloc[i]['workflow'],df.iloc[i]['task'],df.iloc[i]['table_type'])
                send_params['card']['elements'][0]['fields'][-1]['text']['content']="** 监控到数据表为0的小时：** \n{} d {} hour\n {}".format(idate,df.iloc[i]['err_hour'],df.iloc[i]['table_info'])
