...
def send_to_feishu(sid,iname,send_params):
    print('群告警')
    '''正式'''
...
def get_workflow(project,workflow,err_hour):
    fsql_mysql_workflows='''select name,state,run_times,flag,start_time t_ds_process_instance from t_ds_process_instance where project_id=(select id from t_ds_project where name='{}' limit 1) and substring_index(name,'-',1)='{}' order by start_time desc limit {};'''.format(project,workflow,err_hour)
...
if __name__ == '__main__':
 try:
    ...
    fsql_mysql_tables='''SELECT table_name,table_path,err_ds,err_hour,table_info,person_in_charge,project,workflow,task,table_type,flag from sla_test'''
    ...
    for i in range(len(df_mysql_tables)):
        ...
        print(table_name+"\t"+str(file_length)+"\t"+str(table_type))
        ...
        if(table_name.startswith("basicdata.")):
           ...
           if(is_partition_exist=='1'):
               send_to_feishu(sid,iname,send_params)
               send_to_phone()
               send_to_error(table_name,partition,'1b')
           continue
        ...
        if(file_length<100):
           if(flag=='0'):
               ...
               workflow_status=get_workflow(project,workflow,err_hour)
               if(workflow_status=='7' or workflow_status=='1'):
                   print('工作流正常运行!')
               else:
